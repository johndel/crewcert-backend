...
Rails.application.configure do
  ...
  config.action_mailer.default_url_options = { host: ENV.fetch("APP_HOST", "crewcert.xeri.eu"), protocol: "https" }

  # SMTP configuration (supports Mailgun, SendGrid, or any SMTP provider)
  config.action_mailer.delivery_method = :smtp
  config.action_mailer.smtp_settings = {
    user_name: Rails.application.credentials.dig(:smtp, :username) || ENV["SMTP_USERNAME"],
    password: Rails.application.credentials.dig(:smtp, :password) || ENV["SMTP_PASSWORD"],
    address: Rails.application.credentials.dig(:smtp, :host) || ENV.fetch("SMTP_HOST", "smtp.mailgun.org"),
